...
raise NameError('projection_networks_face.py is deprecated. Please use multi_net.py')
...
class EncoderDiscriminatorNet(nn.Module):
	...
	"""
	def __init__(self, opt, discriminatorA_arch, discriminatorB_arch, encoder_archs, discriminatorA_settings={},
				 discriminatorB_settings={}, encoder_settings=None):
		...
		self.discriminatorA_settings = discriminatorA_settings
		self.discriminatorB_settings = discriminatorB_settings
		...
		if encoder_archs:
			...
			if encoder_settings is None:
				encoder_settings = [{}] * len(encoder_archs)
			encoder_modules = []
	...
	def encoder_from_checkpoint(self, checkpoint):
		...
		if 'generator_arch' in checkpoint['opt']:  # old version
			...
			for i, arch in enumerate(checkpoint['encoder_archs']):
				encoder_modules.append(generators.__dict__[arch](checkpoint['opt'], **checkpoint['encoder_settings'][i]))
...
def model_from_checkpoint(checkpoint_path):
	...
	if 'generator_state_dict' in checkpoint:
		...
		encoder_arch = checkpoint['encoder_arch']
